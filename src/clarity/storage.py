...
class Store:
    ...
    def requeue_blocked(self,mission_id,*,new_max_attempts=None):
        ...
        budget=max(r["max_attempts"],int(new_max_attempts or (r["max_attempts"]+1)))
    ...
    def recover_expired(self):
        now=utc_ms(); rows=list(self.conn.execute("SELECT id,attempt,max_attempts FROM missions WHERE state='working' AND lease_until_ms < ?",(now,)))
        recovered=0
        for r in rows:
    ...
    def rebuild_receipts(self):
        ...
        rows=[self._receipt_from_row(r) for r in self.conn.execute("SELECT * FROM events ORDER BY seq")]
        tmp=self.paths.receipts.with_suffix(".jsonl.rebuild")
        with tmp.open("w",encoding="utf-8") as f:
            for r in rows:f.write(canonical_json(r)+"\n")
            f.flush();os.fsync(f.fileno())
        os.replace(tmp,self.paths.receipts)
        d=os.open(self.paths.root,os.O_RDONLY)
        try:os.fsync(d)
        finally:os.close(d)
        return len(rows)
    def create_snapshot(self,label=None):
        ...
        shutil.copy2(self.paths.receipts,target/"receipts.jsonl") if self.paths.receipts.exists() else (target/"receipts.jsonl").write_text("")
        manifest={"schema":"clarity.snapshot.v1","created_ms":utc_ms(),"db_sha256":sha256_bytes(dbcopy.read_bytes()),"receipts_sha256":sha256_bytes((target/"receipts.jsonl").read_bytes())}
        (target/"manifest.json").write_text(canonical_json(manifest)+"\n",encoding="utf-8")
        return target
    @staticmethod
    def restore_snapshot(root,snapshot):
        root=Path(root); snap=Path(snapshot); manifest=json.loads((snap/"manifest.json").read_text())
        db=(snap/"clarity.db").read_bytes(); receipts=(snap/"receipts.jsonl").read_bytes()
        if sha256_bytes(db)!=manifest["db_sha256"] or sha256_bytes(receipts)!=manifest["receipts_sha256"]: raise RuntimeError("snapshot hash mismatch")
        root.mkdir(parents=True,exist_ok=True)
        for name,data in (("clarity.db",db),("receipts.jsonl",receipts)):
            fd,tmp=tempfile.mkstemp(prefix=f".{name}.",dir=root)
            try:
    ...
    def orphan_artifacts(self):
        referenced={str((self.paths.root/r[0]).resolve()) for r in self.conn.execute("SELECT relative_path FROM artifacts")}
        blobs=[]
        for p in self.paths.artifacts.rglob("*"):
            if p.is_file() and str(p.resolve()) not in referenced: blobs.append(p)
        return blobs

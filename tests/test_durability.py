...
def spec():
    return {"logical_name":"april-test-node.json","content":{"schema":"clarity.world-node.v1","id":"april-test-node","offline":True,"requires_model":False}}

def test_idempotent_enqueue(tmp_path:Path):
 s=Store(tmp_path)
 try:
  a=s.add_mission("world.build_slice",spec(),"same-key");b=s.add_mission("world.build_slice",spec(),"same-key");assert a==b;assert len(s.list_missions())==1
 finally:s.close()

def test_expired_lease_recovers(tmp_path:Path):
 s=Store(tmp_path)
 try:
  mid=s.add_mission("world.build_slice",spec());leased=s.lease_next("dead-worker",ttl_ms=1);assert leased["id"]==mid;time.sleep(.01);assert s.recover_expired()==1;assert s.get_mission(mid)["state"]=="queued"
 finally:s.close()

def test_artifact_promotes_only_after_readback(tmp_path:Path):
 s=Store(tmp_path)
 try:
  mid=s.add_mission("world.build_slice",spec());mission=dict(s.lease_next("worker",ttl_ms=10000));result=execute_local(s,mission);s.promote_artifact(mid,mission["lease_token"],result);artifact=s.paths.root/result["relative_path"];assert artifact.exists();assert sha256_bytes(artifact.read_bytes())==result["sha256"];assert s.get_mission(mid)["state"]=="promoted";assert s.conn.execute("SELECT COUNT(*) FROM artifacts WHERE mission_id=?",(mid,)).fetchone()[0]==1
 finally:s.close()
...
def test_receipt_stream_exists_and_is_parseable(tmp_path:Path):
 ...
 try:
  ...
  for line in lines:
   obj=json.loads(line);assert obj["event_hash"];assert obj["type"]
 finally:s.close()
